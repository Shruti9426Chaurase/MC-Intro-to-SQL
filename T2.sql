...
-- 1]Total revenue generated from all bookings
SELECT SUM(TotalPrice) AS TotalRevenue FROM Bookings;

-- 2]Details of customers who booked expensive flights (price > 5000)
SELECT DISTINCT Customers.* FROM Customers
JOIN Bookings ON Customers.CustomerID = Bookings.CustomerID
JOIN Flights ON Bookings.FlightID = Flights.FlightID
WHERE Flights.Price > 5000;

-- 3]Find the most popular hotel (highest bookings)
SELECT Hotels.HotelName, COUNT(Bookings.HotelID) AS BookingCount FROM Hotels
JOIN Bookings ON Hotels.HotelID = Bookings.HotelID GROUP BY Hotels.HotelName
ORDER BY BookingCount DESC LIMIT 1;

-- 4]Average flight price by airline
SELECT Airline, AVG(Price) AS AveragePrice
FROM Flights
GROUP BY Airline
ORDER BY AveragePrice DESC;

-- 5]Payments made using 'Credit Card'
SELECT * 
FROM Payments
WHERE PaymentMethod = 'Credit Card';

-- 6]List customers with multiple bookings
SELECT Customers.FirstName, Customers.LastName, COUNT(Bookings.BookingID) AS TotalBookings
FROM Customers
JOIN Bookings ON Customers.CustomerID = Bookings.CustomerID
GROUP BY Customers.CustomerID, Customers.FirstName, Customers.LastName
HAVING TotalBookings > 1;

-- 7]Revenue by city (based on hotel location):
SELECT Hotels.Location, SUM(Bookings.TotalPrice) AS CityRevenue
FROM Hotels
JOIN Bookings ON Hotels.HotelID = Bookings.HotelID
GROUP BY Hotels.Location
ORDER BY CityRevenue DESC;

-- 8] List of customers who have made more than one booking:
SELECT CustomerID, COUNT(*) AS BookingCount
FROM Bookings
GROUP BY CustomerID
HAVING COUNT(*) > 1;

-- 9]Average Spending by Room Type
SELECT h.RoomType, AVG(b.TotalPrice) AS AverageSpending
FROM Hotels h
...
SELECT c.CustomerID, c.FirstName, c.LastName, SUM(b.TotalPrice) AS TotalSpent
FROM Customers c
